            ...
            <div class="card">
                <div class="card-body">
                    <div class="card-body">
                        ...
                        <!-- モーダルウィンドウ -->
                        <div class="modal fade" id="{{$song_search["track_id"]}}" tabindex="-1" aria-labelledby="listRegistModalLabel" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form id="searchform" action="{{ route('song.regist',['song_list' => $id]) }}" method="post" enctype="multipart/form-data">
                                            @csrf
                                            <div class="mb-3">
                                                <p>{{$song_search["name"]}}</p>
                                                <p>{{$song_search["artist"]}}</p>
                                                <input type="hidden" name="name" value="{{$song_search["name"]}}">
                                                <input type="hidden" name="artist" value="{{$song_search["artist"]}}">
                                                <label for="song-point" class="col-form-label" required>点数</label>
                                                <input id="song-input" type="text" class="form-control" name="points" required>
                                                <label for="mp3_data" class="col-form-label">音声ファイル登録</label>
                                                <input id="comment" type="file" class="form-control" name="mp3_data" accept="audio/*">
                                                <label for="song-comment" class="col-form-label">コメント</label>
                                                <textarea id="song-comment" class="form-control" name="comment"></textarea>
                                                <button id="song-button" type="submit" class="btn btn-primary send-button">登録</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
